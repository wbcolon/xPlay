...
void xMovieLibrary::scanForUnknownEntries(const std::list<std::tuple<QString, QString, QString>>& listEntries) {
    ...
    std::vector<std::pair<QString,QString>> currentMovies;
    for (const auto& entry : listEntries) {
        // We need to cache the entries if possible. List should be sorted by tag and directory.
        if ((currentTag != std::get<0>(entry)) || (currentDirectory != std::get<1>(entry))) {
            currentTag = std::get<0>(entry);
            currentDirectory = std::get<1>(entry);
            try {
        ...
        auto entryMovie = std::get<2>(entry);
        auto currentMoviePos = std::find_if(currentMovies.begin(), currentMovies.end(),
                                             [&entryMovie](const std::pair<QString,QString>& elem) {
                                                 return (elem.first == entryMovie);
                                             });
